/*
Code to perform data analysis on the PBC data set in Davies, Coolen and Galla (2021) for retarded kernel model A.
Written by A. Davies (2021).
20 sets of training and test data (obtained from the R code PBC_Split_Average.R) are read in.
Retarded Kernel Model A is fitted to the training data using maximum likelihood.
...
For the model that treats the two events as a composite event we simply read in the files Censor_comp.txt instead of Censor.txt from the training and test folders.
These files are commented out and labelled 'Composite Event Model'. 

Description of retarded kernel model A can be found in the manuscriupt 'Retarded kernels for longitudinal survival analysis and dynamic predictions' Davies, Coolen and Galla (2021). Detailed derivations of the functions evaluated by this code can be found in the corresponding Supplementary Material.

This code took ~ 40 mins to run
*/
#include <iostream>
#include <iomanip>
#include<random>
...
const int p {3}; //number of longitudinal covariates
const int q {1}; //number of fixed covariates

//variables and vectors for training data:
const int N_train {156}; 	//no. of individuals in the training data set
...
const int N_test {156};		//no. of individuals in the test data set
...
/*
To perform maximum likelihood inference of retarded kernel model A we minimise the negative log likelihood for the training data.
...
We make use of Eq (S14) in the correpsonding supplementary material which gives the result of the integration in (3.22) when using the nearest neighbour interpolation procedure for Model A.
*/
	...
	for (int mu = 0; mu < p; mu++) {
		...
		for(int a = 0; a < n[j]; a++){	
			if(s_j==0){
				...
				double numer = exp(-(min_XS-min_XT)/V[tau])-exp(-(min_XS-T[sum_nj+j+a])/V[tau]);
				double denom = 1.0 - exp(-min_XS/V[tau]);
...
Doub mu_sum2(VecDoub V, vector<double> T, vector<double> Z, VecDoub Z_fix, vector<double> X, int i, int p) {
	...
	for (int mu = 0; mu < p; mu++) {
		...
		for(int a = 0; a < n[i]; a++){	
			...
			else if (X[i]<=T[sum_ni+i+a]){
				mu_sum = mu_sum;
			}
			else{			
				double min_XT = min(X[i],T[sum_ni+i+a+1]);
				
				double numer = exp(-(min_XS-min_XT)/V[tau])-exp(-(min_XS-T[sum_ni+i+a])/V[tau]);
				double denom = 1.0 - exp(-min_XS/V[tau]);
...
/*
Functions to calculate the prediction error for retarded kernel model A fitted to training data evaluated on test data.
...
*/
double EXP_A(double ti, VecDoub kappa, VecDoub tau, VecDoub gamma, vector<double> Z_t, vector<double> T_t, VecDoub Zf_t, VecInt n_t, int j){
	double sum = 0.0;
	...
	double s_j = T_t[sum_nj + j + n_t[j]]; //the final obs time before baseTime = final switch time (when a=n[k])
	...
	for (int mu = 0; mu < p; mu++) {
		...
		for(int a = 0; a < n_t[j]; a++){	
			if(s_j==0){
				...
				double numer = exp(-(min_XS-min_XT)/tau[mu])-exp(-(min_XS-T_t[sum_nj+j+a])/tau[mu]);
				double denom = 1.0 - exp(-min_XS/tau[mu]);
				
				sum = sum + (kappa[mu]*Z_t[sum_nj*p + a*p + mu]*numer)/denom;
			}
		} //end sum over a
	} //end sum over mu
	//sum over q fixed covariates
...
double S_t(VecDoub kappa, VecDoub tau, VecDoub gamma, double predTime, double baseTime, vector<double> X, VecDoub Z_fix, VecDoub Z_fix_test, vector<double> Z, vector<double> Z_new, vector<double> T, vector<double> T_new, VecInt n, VecInt n_new, VecInt Censor, int j, int N_train){ 
	...
	for(int i=0; i<N_train; i++){ 
		if(Censor[i]==0 || X[i] < baseTime || X[i] > predTime){ 
			...
			double numer = EXP_A(X[i], kappa, tau, gamma, Z_new, T_new, Z_fix_test, n_new, j);
...
int main() {
	...
	//base folder where training and test data are stored for PBC data
	string fn_base = "~.../PBC_DATA/";
	...
	for(int loop=0; loop<rep; loop++){
		...
		//create vectors for each of the longitudinal covariates
		vector<double> Z_bili(sum_n);
		vector<double> Z_alb(sum_n);
		vector<double> Z_proth(sum_n);
		
		int zl_bil = 0;
		double value3_bil;
		ifstream file3_bil;
		file3_bil.open(fn_base + label + "/TRAIN/Zlong_bili.txt");
		while (file3_bil >> value3_bil)
		{
			Z_bili[zl_bil] = value3_bil;   
			zl_bil++;
		}
		...
		double value3_alb;
		ifstream file3_alb;
		file3_alb.open(fn_base + label + "/TRAIN/Zlong_alb.txt");
		while (file3_alb >> value3_alb)
		{
			Z_alb[zl_alb] = value3_alb;  
			zl_alb++;
		}
		...
		double value3_pro;
		ifstream file3_pro;
		file3_pro.open(fn_base + label + "/TRAIN/Zlong_proth.txt");
		while (file3_pro >> value3_pro)
		{
			Z_proth[zl_pro] = value3_pro;  
			zl_pro++;
		}
		...
		double value4; 
		ifstream file4;
		file4.open(fn_base + label + "/TRAIN/Zfix_age.txt");
		while (file4 >> value4)
		{
			Z_fix[zf] = value4;   
			zf++;
		...
		int value6;
		ifstream file6;
		file6.open(fn_base + label + "/TRAIN/Censor.txt");		//transplant = censor event model
		//file6.open(fn_base + label + "/TRAIN/Censor_comp.txt");	//composite event model
		while (file6 >> value6)
		{
			Censor[c] = value6;   
		...
		//create vectors for each of the longitudinal covariates
		vector<double> Z_bili_test(sum_n_test);
		vector<double> Z_alb_test(sum_n_test);
		vector<double> Z_proth_test(sum_n_test);

		int zl_bil2 = 0;
		double value3_bil2;
		ifstream file3_bil2;
		file3_bil2.open(fn_base + label + "/TEST/Zlong_bili.txt");
		while (file3_bil2 >> value3_bil2)
		{
			Z_bili_test[zl_bil2] = value3_bil2;   
			zl_bil2++;
		}
		...
		double value3_alb2;
		ifstream file3_alb2;
		file3_alb2.open(fn_base + label + "/TEST/Zlong_alb.txt");
		while (file3_alb2 >> value3_alb2)
		{
			Z_alb_test[zl_alb2] = value3_alb2;   
			zl_alb2++;
		}
		...
		double value3_pro2;
		ifstream file3_pro2;
		file3_pro2.open(fn_base + label + "/TEST/Zlong_proth.txt");
		while (file3_pro2 >> value3_pro2)
		{
			Z_proth_test[zl_pro2] = value3_pro2;   
			zl_pro2++;
		}
		...
		for(int i=0; i<N_test; i++){
			int sum_ni=0;
			for(int k=0; k<i; k++){
				sum_ni = sum_ni + n_test[k];
			}
			for(int a=0; a<n_test[i]; a++){
				Z_test[sum_ni*p+a*p+0] = log(Z_bili_test[sum_ni+a]);
				Z_test[sum_ni*p+a*p+1] = log(Z_alb_test[sum_ni+a]);
				Z_test[sum_ni*p+a*p+2] = log(Z_proth_test[sum_ni+a]);
			}
		}	
		...
		double value4a; 
		ifstream file4a;
		file4a.open(fn_base + label + "/TEST/Zfix_age.txt");
		while (file4a >> value4a)
		{
			Z_fix_test[zfa] = value4a;   
			zfa++;
		}
		...
		int value6a;
		ifstream file6a;
		file6a.open(fn_base + label + "/TEST/Censor.txt");		//transplant = censor event model
		//file6a.open(fn_base + label + "/TEST/Censor_comp.txt");	//composite event model
		...
		try{
			...
			cout << "-F(end) = "  << setprecision(15) << powell.fret << endl;
			...
			ofstream filePT;
			filePT.open(fn_base+label+"/PredTime_fixt.txt");
			ofstream filePE;
			filePE.open(fn_base+label+"/ModelA_fixt.txt");

			double baseTime = 3.0;
			for(int l=0; l<26; l++){
				double predTime = baseTime+(double)l*0.2;
				filePT << to_string(predTime) << "\n";
			...
			ofstream filePT1;
			filePT1.open(fn_base+label+"/BaseTime_w1.txt");
			ofstream filePE1;
			filePE1.open(fn_base+label+"/ModelA_w1.txt");
			...
			ofstream filePT2;
			filePT2.open(fn_base+label+"/BaseTime_w2.txt");
			ofstream filePE2;
			filePE2.open(fn_base+label+"/ModelA_w2.txt");
			...
			ofstream filePT3;
			filePT3.open(fn_base+label+"/BaseTime_w3.txt");
			ofstream filePE3;
			filePE3.open(fn_base+label+"/ModelA_w3.txt");
			...
			cout << "can't minimise LOOP " << loop + 1 << endl;
		}
