/*
Code to perform data analysis on AIDS data set in Davies, Coolen and Galla (2021) for retarded kernel model A.
Written by A. Davies (2021).
20 sets of training and test data (obtained from the R code AIDS_Split_Average.R) are read in.
...
This code took ~ 1hr 13 mins to run
*/
...
const int q {4}; //number of fixed covariates

//variables and vectors for training data:
const int N_train {233}; 	//no. of individuals in the training data set
...
const int N_test {234};		//no. of individuals in the test data set
...
int main() {
	...
	//base folder where training and test data are stored for AIDS data
	string fn_base = "~.../AIDS_DATA/";
	...
	for(int loop=0; loop<rep; loop++){
		...
		double value3;
		ifstream file3;
		file3.open(fn_base + label + "/TRAIN/Zlong_CD4.txt");
		while (file3 >> value3)
		{
			Z[zl] = value3;   
			zl++;
		...
		//Fixed covariates********************************
		//create vectors for each of the q fixed covariates
		VecInt Zf_AZT(N_train);
		VecInt Zf_drug(N_train);
		VecInt Zf_gend(N_train);
		VecInt Zf_prev(N_train);

		int zf = 0;
		int value4; 
		ifstream file4;
		file4.open(fn_base + label + "/TRAIN/Zfix_AZT.txt");
		while (file4 >> value4)
		{
			Zf_AZT[zf] = value4;   
			zf++;
		...
		int value4b; 
		ifstream file4b;
		file4b.open(fn_base + label + "/TRAIN/Zfix_drug.txt");
		while (file4b >> value4b)
		{
			Zf_drug[zfb] = value4b;  
			zfb++;
		}
		...
		int value4c; 
		ifstream file4c;
		file4c.open(fn_base + label + "/TRAIN/Zfix_gender.txt");
		while (file4c >> value4c)
		{
			Zf_gend[zfc] = value4c;	
			zfc++;
		}
		...
		int value4d; 
		ifstream file4d;
		file4d.open(fn_base + label + "/TRAIN/Zfix_prevOI.txt");
		while (file4d >> value4d)
		{
			Zf_prev[zfd] = value4d;	
			zfd++;
		}

		//now create full Z_fix
		for(int i=0; i<N_train; i++){
			Z_fix[q*i + 0] = (double)Zf_AZT[i];
			Z_fix[q*i + 1] = (double)Zf_drug[i];
			Z_fix[q*i + 2] = (double)Zf_gend[i];
			Z_fix[q*i + 3] = (double)Zf_prev[i];
		}
		//End fixed covariates****************************
		...
		double value3a;
		ifstream file3a;
		file3a.open(fn_base + label + "/TEST/Zlong_CD4.txt");
		while (file3a >> value3a)
		{
			Z_test[zla] = value3a;   //assigns to the array
			zla++;
		}
		...
		//Fixed covariates********************************
		//create vectors for each of the q fixed covariates
		VecInt Zf_AZT_test(N_test);
		VecInt Zf_drug_test(N_test);
		VecInt Zf_gend_test(N_test);
		VecInt Zf_prev_test(N_test);

		int zfa = 0;
		int value4a; //will need to change if fixed covariates aren't all int
		ifstream file4a;
		file4a.open(fn_base + label + "/TEST/Zfix_AZT.txt");
		while (file4a >> value4a)
		{
			Zf_AZT_test[zfa] = value4a;   
			zfa++;
		}


		int zfba = 0;
		int value4ba; //will need to change if fixed covariates aren't all int
		ifstream file4ba;
		file4ba.open(fn_base + label + "/TEST/Zfix_drug.txt");
		while (file4ba >> value4ba)
		{
			Zf_drug_test[zfba] = value4ba;   
			zfba++;
		}

		int zfca = 0;
		int value4ca; //will need to change if fixed covariates aren't all int
		ifstream file4ca;
		file4ca.open(fn_base + label + "/TEST/Zfix_gender.txt");
		while (file4ca >> value4ca)
		{
			Zf_gend_test[zfca] = value4ca;	
			zfca++;
		}

		int zfda = 0;
		int value4da; //will need to change if fixed covariates aren't all int
		ifstream file4da;
		file4da.open(fn_base + label + "/TEST/Zfix_prevOI.txt");
		while (file4da >> value4da)
		{
			Zf_prev_test[zfda] = value4da;	
			zfda++;
		}

		//now create full Z_fix
		for(int i=0; i<N_test; i++){
			Z_fix_test[q*i + 0] = (double)Zf_AZT_test[i];
			Z_fix_test[q*i + 1] = (double)Zf_drug_test[i];
			Z_fix_test[q*i + 2] = (double)Zf_gend_test[i];
			Z_fix_test[q*i + 3] = (double)Zf_prev_test[i];
		}
		//End Fixed covariates****************************
